    ...
    for bdengine_file in bdengine_files:
        ...
        for line in results:
            ...
            if (not first_file and selector_key in previous_tags and
                ...
                line = re.sub(r"interpolation_duration: \d+, ", "", line)
                line = line.replace("data merge entity", "item replace entity")
                line = re.sub(r'\{item:\{id:player_head,components:\{"minecraft:', 'container.0 with player_head[', line)
                line = re.sub(r'":\{properties:\[\{name:textures,value:', '={properties:[{name:"textures",value:', line)
                line = re.sub(r'\}\]\}\}\}\}', '}]}]', line)
