...
namespace FileSystem
{
    public class ExportManager : MonoBehaviour
    {
        ...
        // 이전 프레임 상태 저장을 위한 Dictionary
        private Dictionary<string, Matrix4x4> previousTransforms = new Dictionary<string, Matrix4x4>();
        private Dictionary<string, string> previousHeadTextures = new Dictionary<string, string>();
        private HashSet<string> entitiesInPreviousFrame = new HashSet<string>();
        ...
        public async UniTask ExportFile()
        {
            ...
            for (int i = 0; i < sortedTicks.Count; i++)
            {
                int tickValue = sortedTicks[i]; // 해당 프레임의 실제 Tick 값
