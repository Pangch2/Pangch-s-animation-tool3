...
namespace FileSystem
{
    public class ExportManager : BaseManager
    {
        ...
        public async UniTask ExportFile()
        {
            ...
            {
                int scoreVal = exportSetting.startTick + result.Keys[i];
                scoreLines.Add(
        ...
        public void DeleteFrameAndFnumberFiles(string folderPath)
        {
            ...
            string frameFileNameToDelete = string.IsNullOrWhiteSpace(exportSetting.frameFileName) ? "frame.mcfunction" : $"{exportSetting.frameFileName}.mcfunction";
